...
# [关键] 输出文件夹名称 (建议区分大信号和小信号)
OUTPUT_FOLDER_NAME = 'yolo_train_set_v4_500_small' 
...
# --- 负样本掺入策略 ---
NEGATIVE_RATIO = 0.06     # 负样本(纯背景)保留比例
...
def main():
    ...
    print(f"--- 预处理开始 ---")
    print(f"模式: {'切片处理 (小信号)' if ENABLE_SLICING else '全图处理 (大信号)'}")
    ...
    def process_split_set(bin_files, img_dst_dir, lbl_dst_dir, desc_text):
        ...
        for bin_file in tqdm(bin_files, desc=desc_text):
            ...
            # --- 统一处理逻辑 (无论切片与否) ---
            # 如果 ENABLE_SLICING=False, slice_coords 只有一个元素 (0, h, 0, w)
            if ENABLE_SLICING:
                slice_coords = get_slice_coordinates(full_h, full_w)
            else:
                slice_coords = [(0, full_h, 0, full_w)]
            ...
            if num_to_save > 0 and file_neg_buffer:
                ...
                for img, name in selected:
                    ...
                    count_neg += 1
                neg_quota_balance -= actual_save # 扣除已使用的配额
