...
# [关键] 输出文件夹名称
OUTPUT_FOLDER_NAME = 'v8_small_1_test_nfft_2' 
...
# --- STFT 信号处理设置 ---
USE_FIXED_RES_IN_FULL_MODE = True      # small 时不起作用
FREQ_RES_KHZ = 5         
OVERLAP_RATIO = 0.5       
USE_DB_SCALE = True
...
def process_stft(iq_signal, fs, nperseg, noverlap, use_db, norm_min, norm_max, NFFT_RATIO):
    f, t, Zxx = stft(iq_signal, fs=fs, window='hann', nperseg=nperseg, noverlap=noverlap, return_onesided=False, nfft=(NFFT_RATIO*nperseg))
...
def main():
    ...
    def process_split_set(bin_files, img_dst_dir, lbl_dst_dir, desc_text):
        ...
        for bin_file in tqdm(bin_files, desc=desc_text):
            ...
            img_rgb, (full_h, full_w) = process_stft(signal, fs, nperseg, noverlap, USE_DB_SCALE, norm_min, norm_max, NFFT_RATIO)
