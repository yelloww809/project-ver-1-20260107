...
# [修正] 路由逻辑
# Large 分支【忽略】的类别 (即这些类只在 Small 分支测)
# 注意：移除了 10，让 Large 分支也能测 10 (10MHz版本)
LARGE_IGNORED_CLASSES = [9, 12, 13] 

# Small 分支【关注】的类别 (即这些类 Small 分支要测)
# 注意：包含了 10 (0.3/0.5MHz版本)
...
class SignalInferenceSystem:
    ...
    def _process_stft(self, iq_signal, fs, freq_res_khz):
        ...
        if nperseg < 64: nperseg = 64 # 保护性保底
        
        noverlap = int(nperseg * OVERLAP_RATIO)
        
        # [对齐 v8] STFT + Shift + Abs + Log
    ...
    def predict(self, bin_path, json_path, conf_thres=0.25, iou_thres=0.45):
        ...
        # 3. 推理 (Large) - 整图
        # [修正] imgsz=640, rect=False 以匹配训练条件，解决大类识别差的问题
        ...
        if res_large.boxes is not None:
            ...
            if boxes_large_data is not None and len(boxes_large_data) > 0:
                ...
                for box in boxes_large_np:
                    cls_id = int(box[5])
                    # [修正] 只有 LARGE_IGNORED_CLASSES (9,12,13) 被跳过，10 会被保留
        ...
        for box in final_small_preds:
            cls_id = int(box[5])
            # [修正] 只有 SMALL_TARGET_CLASSES (9,10,12,13) 被处理
