...
# 类别定义 (Small 分支负责的难样本)
SMALL_BRANCH_CLASSES = [9, 10, 12, 13] 
...
class SignalInferenceSystem:
    ...
    def _process_stft(self, iq_signal, fs, freq_res_khz):
        """生成 STFT 图 (RGB, 0-255)"""
    ...
    def predict(self, bin_path, json_path, conf_thres=0.25, iou_thres=0.45):
        """核心推理流程"""
        # 1. 读取数据
        ...
        # 2. 预处理 (两路)
        img_large, shape_large = self._process_stft(iq_signal, fs, FREQ_RES_LARGE)
        h_L, w_L = shape_large[0], shape_large[1]
        
        img_small_full, shape_small = self._process_stft(iq_signal, fs, FREQ_RES_SMALL)
        h_S, w_S = shape_small[0], shape_small[1]
        
        # 3. 推理 (Large) - 整图
        res_large_list = self.model_large.predict(img_large, conf=conf_thres, iou=iou_thres, device=self.device, verbose=False, imgsz=1280, rect=True)
        ...
        for (sy1, sy2, sx1, sx2) in slice_coords:
            img_slice = img_small_full[sy1:sy2, sx1:sx2]
            res_slice_list = self.model_small.predict(img_slice, conf=conf_thres, iou=iou_thres, device=self.device, verbose=False, imgsz=640)
        ...
        if res_large.boxes is not None:
            ...
            if boxes_large_data is not None and len(boxes_large_data) > 0:
                ...
                for box in boxes_large_np:
                    cls_id = int(box[5])
                    if cls_id not in SMALL_BRANCH_CLASSES: 
                        ts, te, fs, fe = pixel_to_physical(box[:4], w_L, h_L)
                        # [修复] 强制转换为 float，解决 json float32 报错
                        predictions.append({
                            "class": cls_id,
        ...
        for box in final_small_preds:
            cls_id = int(box[5])
            if cls_id in SMALL_BRANCH_CLASSES: 
        ...
        return predictions, img_large, (w_L, h_L), meta
