...
SLICE_SIZE = 640
SLICE_OVERLAP = 0.2
BATCH_SIZE_SMALL = 16  # [新增] Small 分支批量推理的 Batch Size
...
class SignalInferenceSystem:
    def __init__(self, base_model_path=None, large_model_path=None, small_model_path=None, device='cuda:0'):
        print(f">>> Initializing Inference System V8 (SCHEME={SCHEME}, Parallel+Batch)...")
        self.device = device
        self.scheme = SCHEME
        ...
        print(">>> Models Loaded.")
        # 创建线程池，最大 worker 为 2 (因为只有两个分支)
        self.executor = concurrent.futures.ThreadPoolExecutor(max_workers=2)

    def _process_stft_configurable(self, iq_signal, fs, stft_mode, fixed_nperseg, freq_res_khz, 
    ...
    def _predict_large_branch(self, iq_signal, fs, meta, conf_thres, iou_thres):
        """
        处理 Large 分支：STFT -> 推理 -> 坐标解析
        返回: (predictions_list, img_large, (w_L, h_L))
        """
        ...
        if res_large.boxes is not None and res_large.boxes.data is not None:
            ...
            for box in boxes_np:
                ...
                if cls_id not in LARGE_IGNORED_CLASSES: 
                    # 使用内部变量名，避免覆盖 fs
                    x1, y1, x2, y2 = box[:4]
                    t_start = (x1 / w_L) * duration * 1000 
                    t_end   = (x2 / w_L) * duration * 1000 
                    f_start = (y1 / h_L) * bw_mhz + f_min
                    f_end   = (y2 / h_L) * bw_mhz + f_min

                    preds.append({
                        "class": cls_id, 
                        "start_time": float(min(t_start, t_end)), 
                        "end_time": float(max(t_start, t_end)), 
                        "start_frequency": float(min(f_start, f_end)), 
                        "end_frequency": float(max(f_start, f_end)), 
                        "confidence": float(box[4]), 
                        "source": "large"
                    })
        
        return preds, img_large, (w_L, h_L)

    def _predict_small_branch(self, iq_signal, fs, meta, conf_thres, iou_thres):
        """
        处理 Small 分支：STFT -> 切片 -> [Batch推理] -> Stitching -> 坐标解析
        返回: (predictions_list, debug_slices_list, (w_S, h_S))
        """
        obs_range = meta['observation_range']
        f_min, f_max = obs_range[0], obs_range[1]
        bw_mhz = f_max - f_min
        ...
        # 2. Slice Generation (准备 Batch 数据)
        ...
        # 将所有切片收集到一个列表中
        slice_images = []
        slice_info = [] # 记录 offset
        
        for i, (sy1, sy2, sx1, sx2) in enumerate(slice_coords):
            img_slice = img_small_full[sy1:sy2, sx1:sx2]
            slice_images.append(img_slice)
            slice_info.append((sx1, sy1))
            debug_slices.append((img_slice, f"_s{i}"))

        # 3. Batch Inference (关键加速点)
        if len(slice_images) > 0:
            # 直接传递 List[np.ndarray] 给 YOLOv8，它会自动处理 Batching
            # stream=True 在处理大列表时可以节省显存，但这里为了确保同步完成，使用标准调用
            # 也可以手动设置 batch=BATCH_SIZE_SMALL
            results_list = self.model_small.predict(
                slice_images, 
                conf=conf_thres, 
                iou=iou_thres, 
                device=self.device, 
        ...
        # 5. Process Results (Physical Mapping)
        preds = []
        for box in final_small_preds:
            cls_id = int(box[5])
            if cls_id in SMALL_TARGET_CLASSES: 
                x1, y1, x2, y2 = box[:4]
                t_start = (x1 / w_S) * duration * 1000 
                t_end   = (x2 / w_S) * duration * 1000 
                f_start = (y1 / h_S) * bw_mhz + f_min
                f_end   = (y2 / h_S) * bw_mhz + f_min
        ...
        return preds, debug_slices, (w_S, h_S)
    ...
    def predict(self, bin_path, json_path, conf_thres=0.001, iou_thres=0.7):
        ...
        img_vis = None 
        h_vis, w_vis = 0, 0
        duration = len(iq_signal) / fs # Keep for base scheme fallback
        ...
        if self.scheme == 0:
            ...
            if res_base.boxes is not None and res_base.boxes.data is not None:
                ...
                for box in boxes_np:
                    cls_id = int(box[5])
                    x1, y1, x2, y2 = box[:4]
                    t_start = (x1 / w_vis) * duration * 1000 
                    t_end   = (x2 / w_vis) * duration * 1000 
                    f_start = (y1 / h_vis) * bw_mhz + f_min
                    f_end   = (y2 / h_vis) * bw_mhz + f_min
                    
                    predictions.append({
                        "class": cls_id, "start_time": float(min(t_start, t_end)), "end_time": float(max(t_start, t_end)), 
                        "start_frequency": float(min(f_start, f_end)), "end_frequency": float(max(f_start, f_end)), 
                        "confidence": float(box[4]), "source": "base"
                    })

        # ---------------- SCHEME 1: Dual (Parallel) ----------------
        elif self.scheme == 1:
            # 使用线程池并发提交任务
            # 注意：传入 meta, iq_signal, fs 的副本或引用皆可，Python 自动处理
            
            future_large = self.executor.submit(
                self._predict_large_branch, iq_signal, fs, meta, conf_thres, iou_thres
            )
            future_small = self.executor.submit(
                self._predict_small_branch, iq_signal, fs, meta, conf_thres, iou_thres
            )

            # 等待结果 (谁慢等谁)
            large_preds, img_large, dims_large = future_large.result()
            small_preds, slices, dims_small = future_small.result()

            # 合并结果
            predictions.extend(large_preds)
            predictions.extend(small_preds)
            
            # 设置可视化输出 (默认显示 Large 的全景图)
            img_vis = img_large
            w_vis, h_vis = dims_large[0], dims_large[1]
            debug_slices = slices
