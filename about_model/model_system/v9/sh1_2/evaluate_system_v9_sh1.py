...
# 引入 V9 系统
from inference_system_v9_sh1 import SignalInferenceSystem, SCHEME
from ultralytics.utils.metrics import ap_per_class, box_iou

import torch
print(f"CUDA available: {torch.cuda.is_available()}")
print(f"Device count: {torch.cuda.device_count()}")  # 这里大概率会输出 1
print(f"Current device: {torch.cuda.current_device()}")
print(f"Device name: {torch.cuda.get_device_name(0)}") #看看这是哪张卡

# ================= 配置区域 (V9 独立调优) =================
TEST_DIR = Path(r'E:\huangwenhao\processed_datasets\dataset_test')
OUTPUT_DIR = Path(rf'E:\huangwenhao\results\v9\system_v9\test_v9_sh{SCHEME}_2')
...
# [V9 独立阈值设置]
# Base 方案 (Scheme 0)
METRIC_CONF_BASE = 0.20
VIS_CONF_BASE    = 0.20

# Dual 方案 (Scheme 1)
# Large 分支：宽带/微弱信号，需要低阈值召回
METRIC_CONF_LARGE = 0.10
VIS_CONF_LARGE    = 0.10

# Small 分支：窄带/拼接，建议稍高以减少碎片误检
METRIC_CONF_SMALL = 0.60  # 即使是 Metric 也建议设高一点，防止拼接逻辑被噪声干扰
VIS_CONF_SMALL    = 0.60

# ================= 混淆矩阵类 =================
class ConfusionMatrix:
    def __init__(self, num_classes, iou_thres=0.5):
        # [V9 Change] 移除内部 conf_thres，现在由外部过滤后传入
        self.nc = num_classes
        self.iou = iou_thres
    ...
    def process_batch(self, preds, gts):
        ...
        pred_t = to_scaled(preds, is_gt=False)
...
def process_batch_metrics_strict(preds, gts, meta, iou_thresholds):
    # 逻辑保持不变，preds 已经由推理系统根据 Metric Conf 过滤过
    SCALE_TIME = 1000.0 
    SCALE_FREQ = 1.0
...
def main():
    ...
    cm = ConfusionMatrix(num_classes=14, iou_thres=0.5)
    
    print(f">>> Start Testing V9 (SCHEME={SCHEME}) on {len(bin_files)} files...")
    ...
    for bin_file in tqdm(bin_files):
        ...
        # 1. 预测 (使用独立的 Metric Thres)
        preds, img_vis, img_dims, meta, slices = system.predict(
            bin_file, json_file, 
            conf_base=METRIC_CONF_BASE,
            conf_large=METRIC_CONF_LARGE,
            conf_small=METRIC_CONF_SMALL,
            iou_thres=0.6
        )
        
        gts = meta.get('signals', [])
        
        # 2. 计算指标 (基于原始 Metric Conf 的预测结果)
        tp, conf, pcls, tcls = process_batch_metrics_strict(preds, gts, meta, iou_v)
        stats.append((tp.cpu(), conf.cpu(), pcls.cpu(), tcls.cpu()))
        
        # 3. 结果过滤 (针对 Visualization 和 Confusion Matrix)
        # 这是一个关键步骤：从 Metric 结果中筛选出高置信度的 Vis 结果
        vis_preds_filtered = []
        for p in preds:
            c = p['confidence']
            src = p['source']
            if src == 'base':
                if c >= VIS_CONF_BASE: vis_preds_filtered.append(p)
            elif src == 'large':
                if c >= VIS_CONF_LARGE: vis_preds_filtered.append(p)
            elif src == 'small':
                if c >= VIS_CONF_SMALL: vis_preds_filtered.append(p)

        # 更新混淆矩阵 (使用过滤后的结果)
        cm.process_batch(vis_preds_filtered, gts)
        ...
        if is_visualizing:
            ...
            system.save_results(vis_preds_filtered, sample_dir / f"{bin_file.stem}_pred.json")
    ...
    if tp.shape[0] > 0:
        ...
        with open(result_txt_path, 'w', encoding='utf-8') as f:
            f.write(f"SCHEME: {SCHEME} (V9 Independent Thresholds)\n")
            f.write(f"METRIC_CONF: Base={METRIC_CONF_BASE}, Large={METRIC_CONF_LARGE}, Small={METRIC_CONF_SMALL}\n")
            f.write("-" * 90 + "\n")
