...
SLICE_SIZE = 640
SLICE_OVERLAP = 0.2
BATCH_SIZE_SMALL = 16  # [V8/V9 Feature] Small 分支批量推理的 Batch Size
...
class SignalInferenceSystem:
    def __init__(self, base_model_path=None, large_model_path=None, small_model_path=None, device='cuda:0'):
        print(f">>> Initializing Inference System V9 (SCHEME={SCHEME}, Parallel+Batch+SplitConf)...")
        self.device = device
        self.scheme = SCHEME
    ...
    # ================= 主预测入口 (V9 Upgrade) =================
    def predict(self, bin_path, json_path, 
                conf_base=0.001, conf_large=0.001, conf_small=0.001, 
                iou_thres=0.7):
        """
        [V9] 接收独立的置信度阈值参数。
        """
        ...
        if self.scheme == 0:
            ...
            res_base = self.model_base.predict(
                img_base, conf=conf_base, iou=iou_thres, device=self.device, verbose=False, imgsz=640, rect=False
            )[0]
        ...
        # ---------------- SCHEME 1: Dual (Parallel) ----------------
            ...
            # [V9] 将对应的 Conf 分发给不同的线程
            future_large = self.executor.submit(
                self._predict_large_branch, iq_signal, fs, meta, conf_large, iou_thres
            )
            future_small = self.executor.submit(
                self._predict_small_branch, iq_signal, fs, meta, conf_small, iou_thres
            )
