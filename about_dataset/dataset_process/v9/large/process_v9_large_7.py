...
PROCESSED_ROOT_DIR = Path(r'E:\huangwenhao\processed_datasets\v9')

# [关键] 输出文件夹名称
OUTPUT_FOLDER_NAME = 'v9_large_7' 
...
# --- STFT 信号处理设置 ---
# 模式选择 (STFT_MODE):
# 1: 固定窗长 (Fixed Window Length) -> 使用 FIXED_NPERSEG
# 2: 自适应窗长-正方形像素 (Adaptive - Square Pixels) -> 试图让时频像素接近正方形 (原 USE_FIXED_RES_IN_FULL_MODE = False)
# 3: 自适应窗长-固定频率分辨率 (Fixed Frequency Resolution) -> 使用 FREQ_RES_KHZ (原 USE_FIXED_RES_IN_FULL_MODE = True)
STFT_MODE = 3

# [模式1 参数]
FIXED_NPERSEG = 1024      # 固定窗长点数

# [模式3 参数]
FREQ_RES_KHZ = 20         # 目标频率分辨率 (kHz)
...
# --- 标签过滤设置 ---
TARGET_SIGNALS_TYPE = 'large'
# === settings ===
BW_TOLERANCE_BIG = 0.5
BW_TOLERANCE_SMALL = 0.002
if TARGET_SIGNALS_TYPE == 'all':
    TARGET_SIGNALS = {
        0:  [20.0],
        1:  [20.0],
        2:  [20.0],
...
elif TARGET_SIGNALS_TYPE == 'large':
        TARGET_SIGNALS = {
        0:  [20.0],
        1:  [20.0],
        2:  [20.0],
...
elif TARGET_SIGNALS_TYPE == 'small':
    TARGET_SIGNALS = {
      9:  [0.0523, 0.0625, 0.25],
      10: [0.3, 0.5],
      12: [0.006, 0.2],
...
def is_target_signal(cls_id, bandwidth):
    ...
    for bw in allowed_bws:
        if TARGET_SIGNALS_TYPE == 'all':
            current_tolerance = BW_TOLERANCE_BIG if bw >= 1.0 else BW_TOLERANCE_SMALL
            if abs(bandwidth - bw) <= current_tolerance:
                return True
        elif TARGET_SIGNALS_TYPE == 'large':
            if abs(bandwidth - bw) <= BW_TOLERANCE_BIG:
                return True
        elif TARGET_SIGNALS_TYPE == 'small':
            if abs(bandwidth - bw) <= BW_TOLERANCE_SMALL:
                return True
        else:
            raise ValueError
    return False
...
def save_initial_settings(output_path):
    ...
    with open(output_path, 'w') as f:
        ...
        # [修改] 记录 STFT 模式
        mode_desc = {
            1: f"Fixed Window Length ({FIXED_NPERSEG})",
            2: "Adaptive (Square Pixels)",
            3: f"Fixed Frequency Resolution ({FREQ_RES_KHZ} kHz)"
        }
        actual_mode = 3 if ENABLE_SLICING else STFT_MODE
        
        f.write(f"STFT Mode\t\t=\t{actual_mode} - {mode_desc.get(actual_mode, 'Unknown')}\n")
        if ENABLE_SLICING:
            f.write(f"  * Slicing Enabled -> Forced to Mode 3\n")
...
def main():
    ...
    print(f"STFT 模式: {STFT_MODE} (1=固定窗长, 2=自适应正方形, 3=固定分辨率)")
    ...
    def process_split_set(bin_files, img_dst_dir, lbl_dst_dir, desc_text):
        ...
        for bin_file in tqdm(bin_files, desc=desc_text):
            ...
            # --- [修改] nperseg 计算逻辑 ---
            # 确定当前实际使用的模式 (切片模式强制为3)
            current_mode = 3 if ENABLE_SLICING else STFT_MODE
            
            nperseg = 256 # 默认值
            
            if current_mode == 1: # Mode 1: 固定窗长
                nperseg = FIXED_NPERSEG
                if nperseg > signal_len: nperseg = signal_len
                
            elif current_mode == 2: # Mode 2: 自适应 (尽量正方形)
                try:
            ...
            elif current_mode == 3: # Mode 3: 固定频率分辨率
