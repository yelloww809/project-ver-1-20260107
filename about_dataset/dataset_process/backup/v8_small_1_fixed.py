...
# [关键] 输出文件夹名称 (建议修改以示区别)
OUTPUT_FOLDER_NAME = 'v8_small_jpg_1_fixed_500' 

# --- 图片保存格式设置 ---
SAVE_IMAGE_FORMAT = 'jpg' 

# --- 采样与数据集划分比例 ---
TOTAL_SAMPLES = 500        # -1: 全部处理
RANDOM_SEED = 42
...
# [核心修复] 最小像素阈值
# 0.9 像素能保留 1.0px 和 1.2px 的信号，同时剔除 0.6px 的切片残渣
MIN_PIXEL_SIZE = 0.9
...
def save_initial_settings(output_path):
    with open(output_path, 'w') as f:
        f.write(f"Dataset Settings (v8 Fixed) - {datetime.now()}\n")
        f.write(f"Note: Applied MIN_PIXEL_SIZE = {MIN_PIXEL_SIZE} filter.\n\n")
...
def main():
    ...
    print(f"--- 预处理开始 (v8 Fixed: Removing Slices < {MIN_PIXEL_SIZE}px) ---")
        ...
        for bin_file in tqdm(bin_files, desc=desc_text):
            ...
            for i, (sy1, sy2, sx1, sx2) in enumerate(slice_coords):
                ...
                for box in valid_boxes:
                    ...
                    # 只有宽高都 >= MIN_PIXEL_SIZE (0.9px) 才保留
                    # 这将保留 1.0px/1.2px 的信号，但丢弃 0.6px 的切片残渣
                    if clipped_w >= MIN_PIXEL_SIZE and clipped_h >= MIN_PIXEL_SIZE:
                        cx, cy, w, h = convert_box_to_yolo((ix1-sx1, iy1-sy1, ix2-sx1, iy2-sy1), slice_w, slice_h)
                        # 双重保险：过滤掉归一化后极小的值 (0.001 approx 0.6px in 640)
