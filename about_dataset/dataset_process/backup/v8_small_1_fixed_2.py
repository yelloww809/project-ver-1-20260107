...
# [关键] 输出文件夹名称
OUTPUT_FOLDER_NAME = 'v8_small_jpg_1_fixed_2' 
...
# [核心策略 1] 丢弃阈值 (小于此值直接丢弃，防止切片碎屑)
MIN_VALID_PIXEL = 0.9

# [核心策略 2] 膨胀阈值 (小于此值强制放大到此值)
# 5.0 像素是给 YOLO 的"保命符"，抵抗随机缩放和Mosaic
FORCE_MIN_SIZE_PIXEL = 5.0
...
def convert_box_to_yolo_inflated(box_px, img_w, img_h):
    """
    带有膨胀逻辑的坐标转换
    box_px: (x1, y1, x2, y2) 绝对像素坐标
    """
    x1, y1, x2, y2 = box_px
    ...
    # [膨胀逻辑] 强制最小尺寸
    # 如果小于阈值，强制设为阈值，保持中心点不变
    final_w_px = max(w_px, FORCE_MIN_SIZE_PIXEL)
    final_h_px = max(h_px, FORCE_MIN_SIZE_PIXEL)
    ...
    return cx_px * dw, cy_px * dh, final_w_px * dw, final_h_px * dh
...
def save_initial_settings(output_path):
    with open(output_path, 'w') as f:
        f.write(f"Dataset Settings (v8 Inflated) - {datetime.now()}\n")
        f.write(f"Filter: Drop < {MIN_VALID_PIXEL} px\n")
        f.write(f"Inflation: Force Min Size >= {FORCE_MIN_SIZE_PIXEL} px\n\n")
        ...
        if USE_FIXED_RES_IN_FULL_MODE or ENABLE_SLICING:
...
def main():
    ...
    print(f"--- 预处理开始 (v8 Inflated: Drop <0.9px, Inflate to 5.0px) ---")
    ...
    def process_split_set(bin_files, img_dst_dir, lbl_dst_dir, desc_text):
        ...
        for bin_file in tqdm(bin_files, desc=desc_text):
            ...
            if ENABLE_SLICING or USE_FIXED_RES_IN_FULL_MODE:
            ...
            for i, (sy1, sy2, sx1, sx2) in enumerate(slice_coords):
                ...
                for box in valid_boxes:
                    ...
                    # 1. 过滤掉 0.9px 以下的碎屑
                    if clipped_w >= MIN_VALID_PIXEL and clipped_h >= MIN_VALID_PIXEL:
                        # 2. 计算膨胀后的坐标
                        # 传入切片内的相对像素坐标，内部会执行 max(w, 5.0)
                        cx, cy, w, h = convert_box_to_yolo_inflated((ix1-sx1, iy1-sy1, ix2-sx1, iy2-sy1), slice_w, slice_h)
